\def\IfInclude#1{%
\openin15=#1\relax%
\ifeof15%
\else\closein15%
\input #1%
\fi%
}

%%%%%%%%%%%%%% Mod keys %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\Ctrl{c}
\def\Shift{s}
\def\Super{w}


%%%%%%%%%%%%%% Undef key %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\UndefKey{}

%%%%%%%%%%%%%% general Commands %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\GoMode#1{\UndefKey}
\def\Cmd#1{\UndefKey}

%%%%%%%%%%%%%% WM Commands %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% workspaces
\def\GoWorkspace#1{\UndefKey}
\def\ContWorkspace#1{\UndefKey}
\def\ContGoWorkspace#1{\And{\ContWorkspace{#1}}{\GoWorkspace{#1}}}

\def\GoMasterWorkspace#1{\UndefKey}
\def\ContMasterWorkspace#1{\UndefKey}
\def\ContGoMasterWorkspace#1{\And{\ContMasterWorkspace{#1}}{\GoMasterWorkspace{#1}}}

\def\GoSlaveWorkspace{\GoWorkspace}
\def\ContSlaveWorkspace{\ContWorkspace}
\def\ContGoSlaveWorkspace#1{\And{\ContSlaveWorkspace{#1}}{\GoSlaveWorkspace{#1}}}

\def\MoveWorkspace#1{\UndefKey}
% Container
\def\FocusWind#1{\UndefKey}
\def\MoveCont#1{\UndefKey}
\def\SplitContV{\UndefKey}
\def\SplitContH{\UndefKey}
\def\SplitContT{\UndefKey}
\def\FullscreenContToggle{\UndefKey}
\def\LayoutCont#1{\UndefKey}
\def\ResizeCont#1#2#3{\UndefKey}
\def\FloatingToggleCont{\UndefKey}
\def\StickyToggleCont{\UndefKey}
% Restart i3
\def\RestartProg{\UndefKey}
\def\RestartProgII{\RestartProg}

%%%%%%%%%%%%%% System modificator %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\ExitProg{\UndefKey}
\def\ExitShutdown{\Cmd{poweroff}}
\def\ExitSuspend{\Cmd{systemctl suspend}}
\def\ExitRestart{\Cmd{reboot}}
\def\Light#1#2{\Cmd{light #1 #2; lightGUI}}
\catcode37=11
\def\Volume#1#2{\Cmd{amixer sset Master -q #2%#1;   sleep 0.05; volumeGUI}}
\catcode37=14
\def\CpuFSet#1{\Cmd{cpuF #1; cpuFGUI 'SET TO #1'}}
\def\CpuF{\Cmd{cpuFGUI}}
\def\DiskSleep#1#2{\Cmd{diskSleepGUI #1 #2}}

%%%%%%%%%%%%%% Not bind commmand %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\And#1#2{#1}
\def\AndII#1#2{#2}
\def\RunCmd#1{}
\def\IiiiLine#1{}
\def\IiiiInline#1{}
\def\SpectrLine#1{}
\def\SpectrInline#1{}
